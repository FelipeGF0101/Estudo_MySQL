...
1 - TRAGA OS FUNCIONÁRIOS QUE TRABALHEM NO DEPARTAMENTO DE FILMES 'OU' NO DEPARTAMENTO DE ROUPAS

2 - O GESTOR DE MARKETING PEDIU A LISTA DAS FUNCIONÁRIAS QUE TRABALHEM NO DEPARTAMENTO DE FILMES 'OU' 
NO DEPARTAMENTO LAR. ELE NECESSITA ENVIAR UM EMAIL PARA AS COLABORADORAS DESSES DOIS SETORES.

3 - COMO ESTAMOS TRABALHANDO COM AND, COLOCAMOS O DEPARTAMENTO PRIMEIRO, POIS ELE IRÁ CHECAR A 
SEGUNDA CONDIÇÃO UM NÚMERO MENOR DE VEZES.

4 - TRAGAM OS FUNCIONÁRIOS DO SEXO MASCULINO 'OU' OS FUNCIONÁRIOS QUE TRABALHEM NO JARDIM.
...
SELECT NOME, SEXO, DEPARTAMENTO FROM FUNCIONARIOS WHERE SEXO = 'MASCULINO' AND (DEPARTAMENTO = 'FILMES' OR DEPARTAMENTO = 'ROUPAS');

/*
mysql> SELECT NOME, SEXO, DEPARTAMENTO FROM FUNCIONARIOS WHERE SEXO = 'MASCULINO' AND (DEPARTAMENTO = 'FILMES' OR DEPARTAMENTO = 'ROUPAS');
+------------+-----------+--------------+
...
/*
PARA SABER QUANTOS FUNCIONÁRIO TEMOS EM CADA DEPARTAMENTO, DEVEMOS UTILIZAR O GROUP BY. ASSIM
CONSEGUIMOS FAZER UMA BUSCA NA TABELA SEM ESTRAGAR A PERFORMANCE DA TABELA.
*/

SELECT COUNT(*), DEPARTAMENTO FROM FUNCIONARIOS GROUP BY DEPARTAMENTO;

/*
mysql> SELECT COUNT(*), DEPARTAMENTO FROM FUNCIONARIOS GROUP BY DEPARTAMENTO;
+----------+--------------+
| COUNT(*) | DEPARTAMENTO |
+----------+--------------+
|       52 | Computadores |
...
TAMBÉM É POSSÍVEL ORDENAR OS VALORES DA COLUNA:
SELECT COUNT(*), DEPARTAMENTO FROM FUNCIONARIOS GROUP BY DEPARTAMENTO ORDER BY 1;
O COUNT(*) É A COLUNA 1, POR ISSO 'O ORDER BY 1'

mysql> SELECT COUNT(*), DEPARTAMENTO FROM FUNCIONARIOS GROUP BY DEPARTAMENTO ORDER BY 1;
+----------+--------------+
| COUNT(*) | DEPARTAMENTO |
+----------+--------------+
|       21 | Filmes       |
...
/*
A FORMA MAIS CORRETA DE RESOLVER A QUESTÃO 1 SERIA:
COMO ESTAMOS TRABALHANDO COM OR E A SEGUNDA CONDIÇÃO É OPCIONAL, COLOCAMOS NA PRIMEIRA CONDIÇÃO QUEM 
TEM MAIS CHANCES DE UMA SAÍDA VERDADEIRA, POIS A SEGUNDA CONDIÇÃO NÃO SERÁ CHECADA NESSE CASO.

SELECT * FROM FUNCIONARIOS WHERE DEPARTAMENTO  = 'ROUPAS' OR DEPARTAMENTO = 'FILMES';

mysql> SELECT * FROM FUNCIONARIOS WHERE DEPARTAMENTO  = 'ROUPAS' OR DEPARTAMENTO = 'FILMES';
+---------------+------------+-----------------------------------+-----------+--------------+------------+---------+-------------------------------+----------+
...
SELECT * FROM FUNCIONARIOS WHERE SEXO = 'FEMININO' AND (DEPARTAMENTO = 'FILMES' OR DEPARTAMENTO = 'LAR');

/*
mysql> SELECT NOME, SEXO, DEPARTAMENTO, EMAIL FROM FUNCIONARIOS WHERE SEXO = 'FEMININO' AND (DEPARTAMENTO = 'FILMES' OR DEPARTAMENTO = 'LAR');
+------------+----------+--------------+-----------------------------------+
...
/*
CONTANDO QUANTAS MULHERES TEMOS NO BANCO

SELECT COUNT(*), SEXO FROM FUNCIONARIOS GROUP BY SEXO;

mysql> SELECT COUNT(*), SEXO FROM FUNCIONARIOS GROUP BY SEXO;
+----------+-----------+
| COUNT(*) | SEXO      |
+----------+-----------+
|      491 | Feminino  |
...
SELECT * FROM FUNCIONARIOS WHERE SEXO = 'FEMININO' AND (DEPARTAMENTO = 'LAR' OR DEPARTAMENTO = 'FILMES');

mysql> SELECT * FROM FUNCIONARIOS WHERE SEXO = 'FEMININO' AND (DEPARTAMENTO = 'LAR' OR DEPARTAMENTO = 'FILMES');
+---------------+------------+-----------------------------------+----------+--------------+------------+---------+-------------------------------+----------+
| idFuncionario | nome       | email                             | sexo     | departamento | admissao   | salario | cargo                         | idRegiao |
+---------------+------------+-----------------------------------+----------+--------------+------------+---------+-------------------------------+----------+
...
/*
mysql> SELECT NOME, SEXO, DEPARTAMENTO FROM FUNCIONARIOS WHERE SEXO = 'MASCULINO' OR DEPARTAMENTO = 'JARDIM';
+------------+-----------+--------------+
| NOME       | SEXO      | DEPARTAMENTO |
+------------+-----------+--------------+
